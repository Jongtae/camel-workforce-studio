...
""",
    ...
    "ux": {
        ...
        "default_topic": "포럼 서비스의 UI/UX를 사용자가 실제로 쓰고 싶어지도록 먼저 닫아야 할 가장 작은 slice는 무엇이며, thread continuity / reply context / compact compose entrypoint / tag navigation / feed clarity / empty state / navigation coherence / copy tone / visual hierarchy 중 무엇을 우선해야 하는가?",
...
- forum 서비스의 첫인상은 thread continuity, reply context, compact compose entrypoint, tag navigation, feed clarity, empty state, navigation coherence, copy tone, visual hierarchy에서 결정된다
...
- thread continuity, reply context, compact compose entrypoint, tag navigation, feed clarity, empty state, navigation coherence, copy tone, visual hierarchy를 반드시 다루라
...
""",
        ...
        "round_focus_guide": {
            1: "사용자가 실제로 쓰고 싶어지게 만드는 가장 큰 surface gap을 찾되, 이번 run에서 issue-ready로 압축할 수 있는 최소 UX slice를 먼저 고르라.",
        ...
        "synthesis_prompt": """위 결과를 바탕으로:
1. 사용자가 실제로 쓰고 싶어지는 포럼/관리 화면이 되기 위해 먼저 닫아야 할 UX gap을 추출하고
