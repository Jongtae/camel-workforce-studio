#!/usr/bin/env python3
"""
ad-fashion-forum — Workforce 기반 시뮬레이션 환경 토론 엔진
...
COMMON_COORDINATOR_PROMPT = """당신은 ad-fashion-forum 프로젝트의 시뮬레이션 설계 토론 코디네이터입니다.
...
SCENARIOS = {
    "commitment": {
        ...
        "workforce_description": "ad-fashion-forum Commitment Workforce: Project State Analyst, Workforce Selector, Topic Architect, Commitment Critic이 협력하여 다음 논의의 workforce와 토픽을 결정합니다.",
        "participants": "Project State Analyst, Workforce Selector, Topic Architect, Commitment Critic",
        "title": "🎯 ad-fashion-forum — Commitment Workforce",
        "arg_description": "CAMEL Workforce 기반 논의 방향 결정 엔진",
        "build_task_prompt": lambda topic: f"""ad-fashion-forum 프로젝트의 다음 논의 방향을 결정하세요.
...
""",
    ...
    "society": {
        ...
        "workforce_description": "ad-fashion-forum 이용자 조직 시뮬레이션 토론 팀: Society Modeling Lead, Identity & Memory Architect, Community Dynamics Researcher, Simulation Critic이 협력하여 API 기반 forum 위에서 action하는 stateful AI agent backend 요구사항을 도출합니다.",
        "participants": "Society Modeling Lead, Identity & Memory Architect, Community Dynamics Researcher, Simulation Critic",
        "title": "🏢 ad-fashion-forum — 이용자 조직 시뮬레이션 토론",
        "arg_description": "CAMEL Workforce 기반 이용자 조직 시뮬레이션 토론 엔진",
        "build_task_prompt": lambda topic: f"""ad-fashion-forum 프로젝트의 forum 안에서 action하는 AI agent backend 요구사항을 토론하세요.

## 토론 주제
{topic}

## 프로젝트 맥락
- ad-fashion-forum에는 이미 sim-server의 agent-loop, agent-core의 action-space, memory-stack, identity-update-rules, forum-generation이 존재한다
...
""",
    ...
    "core": {
        ...
        "workforce_description": "ad-fashion-forum Development 팀 토론: CTO, Backend Engineer, Product Manager, Development Critic이 협력하여 현재 mock을 실제 동작하는 서비스로 전환하는 핵심 개발 계획을 도출합니다.",
        "participants": "CTO, Backend Engineer, Product Manager, Development Critic",
        "title": "🚀 ad-fashion-forum — Development 팀 토론",
        "arg_description": "CAMEL Workforce 기반 development 팀 토론 엔진",
        "build_task_prompt": lambda topic: f"""ad-fashion-forum 프로젝트에서 현재 mock을 실제 서비스로 전환하는 development 계획을 토론하세요.
...
""",
        "coordinator_prompt": COMMON_COORDINATOR_PROMPT + "\n추가 규칙: 이번 토론은 development 팀이 현재 mock을 실제 서비스로 전환하는 계획에 집중한다. 추상적인 설계보다 실제 구현 가능한 첫 번째 동작 루프와 전환 단계를 우선한다.\n",
        "final_prompt": COMMON_FINAL_SYNTHESIZER_PROMPT + "\n추가 규칙: 최종 결과는 mock-to-service 전환 목표, 첫 번째 구현 범위, 코드베이스 전환 계획을 중심으로 development 계획을 정리하라.\n",
        "round_focus_guide": {
            1: "현재 mock이 왜 아직 서비스가 아닌지와 development 팀의 정확한 범위를 정의하라.",
...
""",
    ...
    "operator": {
        ...
        "workforce_description": "ad-fashion-forum 운영자 조직 토론 팀: Operator Strategy Lead, Measurement & Intervention Architect, Moderation & Policy Designer, Operator Critic이 협력하여 운영자 관점의 자정/모니터링/개선 설계안을 도출합니다.",
        "participants": "Operator Strategy Lead, Measurement & Intervention Architect, Moderation & Policy Designer, Operator Critic",
        "title": "🏢 ad-fashion-forum — 운영자 조직 토론",
        "arg_description": "CAMEL Workforce 기반 운영자 조직 토론 엔진",
        "build_task_prompt": lambda topic: f"""ad-fashion-forum 프로젝트의 운영자 조직(포럼을 모니터링하고 자정하며 개선 포인트를 도출하는 조직)을 토론하세요.
...
""",
        "coordinator_prompt": COMMON_COORDINATOR_PROMPT + "\n추가 규칙: 이번 토론은 운영자 조직 설계에 집중한다. 포럼 구축보다 컨텐츠 자정, 모니터링, 개선 포인트 도출을 위한 관찰 프레임과 개입 레버를 우선한다.\n",
        "final_prompt": COMMON_FINAL_SYNTHESIZER_PROMPT + "\n추가 규칙: 최종 결과는 운영자 조직의 관찰 체계, 자정 정책, 개선 레버, 운영 아티팩트를 우선 정리하라.\n",
        "round_focus_guide": {
            1: "운영자 조직이 먼저 답해야 할 운영 질문과 개입 목표를 정의하라.",
            2: "메트릭, 대시보드, 인터뷰, 트레이스, 정책 레버를 구체화하라.",
            3: "측정 불가능한 개입, 운영 리스크, 정책 충돌, 잘못된 개선 방향을 점검하라.",
            4: "반복 운영 루프와 기능 개선 우선순위를 압축하라.",
            5: "이슈로 옮길 수 있게 운영자 조직 설계안과 개선 계획을 정리하라.",
        },
        "synthesis_prompt": """위 결과를 바탕으로:
1. 운영자 조직이 먼저 고정해야 할 관찰 프레임과 개입 목표를 추출하고
2. 어떤 메트릭, 인터뷰, 트레이스, 자정 정책, 개선 레버가 필요한지 정리하고
3. 어떤 운영 아티팩트와 반복 루프가 있어야 다음 런과 다음 기능 개선을 설계할 수 있는지 우선순위를 정한 뒤
4. 바로 GitHub Issue로 등록 가능한 실행 초안을 작성하라.
...
def synthesize_multi_round_result(
    ...
    prompt = f"""다음은 ad-fashion-forum 프로젝트 주제에 대한 다중 라운드 Workforce 토론 결과다.
...
def create_sprint_issue_body(
    ...
    lines.extend(
        [
            ...
            "## Goal",
            "",
            "이번 sprint에서 수행할 ad-fashion-forum 구현/운영 작업을 정리하고 실행 순서를 맞춘다.",
        ]
    )
    return "\n".join(lines).strip() + "\n"
...
def main(argv=None):
    ...
    parser.add_argument(
        ...
        help="Issue를 생성할 대상 GitHub repo (기본: Jongtae/ad-fashion-forum)",
    )
    parser.add_argument(
        "--issue-type",
        type=str,
