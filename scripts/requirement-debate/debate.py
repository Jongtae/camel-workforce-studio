...
""",
    ...
    "ux": {
        ...
        "default_topic": "포럼 서비스의 전체 UI/UX 레이아웃과 정보 구조를 먼저 닫기 위해 지금 가장 먼저 정해야 할 최소 slice는 무엇이며, 그다음 thread continuity / reply context / compact compose entrypoint / tag navigation / feed clarity / empty state / navigation coherence / copy tone / visual hierarchy 중 무엇을 우선해야 하는가?",
...
- 사용자 화면이 반쯤 만들어진 것처럼 보이면 backend simulation보다 먼저 surface UX를 닫아야 한다
      - forum 서비스의 첫인상은 먼저 전체 레이아웃과 정보 구조에서 결정되고, 그다음 thread continuity, reply context, compact compose entrypoint, tag navigation, feed clarity, empty state, navigation coherence, copy tone, visual hierarchy에서 완성된다
...
- 사용자가 실제로 쓰고 싶어지는 포럼 서비스 경험을 우선하라
      - thread continuity, reply context, compact compose entrypoint, tag navigation, feed clarity, empty state, navigation coherence, copy tone, visual hierarchy를 다루되, 먼저 전체 UI/UX 레이아웃과 정보 구조를 고정하라
...
""",
        ...
        "round_focus_guide": {
            1: "사용자가 실제로 쓰고 싶어지게 만드는 가장 큰 surface gap을 찾되, 먼저 전체 UI/UX 레이아웃과 정보 구조를 고정하고 이번 run에서 issue-ready로 압축할 수 있는 최소 UX slice를 고르라.",
        ...
        "synthesis_prompt": """위 결과를 바탕으로:
1. 사용자가 실제로 쓰고 싶어지는 포럼/관리 화면이 되기 위해 먼저 전체 UI/UX 레이아웃과 정보 구조를 고정한 뒤, 닫아야 할 UX gap을 추출하고
