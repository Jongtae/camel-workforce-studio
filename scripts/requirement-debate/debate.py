...
- 단일 기능이나 단일 slice로 너무 빨리 좁히지 말고, 가능하면 상위 목표와 epic 구도를 먼저 제시한다.
...
- 요구사항 확대가 필요한 상황이면, 다음 workforce가 전체 그림을 먼저 그리도록 하는 토픽을 선호한다.
...
- 요구사항 확대가 목적이라면, Topic은 상위 목표와 epic 구조를 먼저 묻는 문장이어야 한다.
...
""",
        "coordinator_prompt": COMMON_COORDINATOR_PROMPT + "\n추가 규칙: 이번 토론은 forum 안에서 action하는 AI agent backend 요구사항에 집중한다. 추상적인 사회 현상보다 action loop, state schema, memory writeback, observable trace를 우선한다. 상위 epic 구조도 함께 보여줘야 한다. 최소 3개의 capability epic 후보를 먼저 제시하라.\n",
        "final_prompt": COMMON_FINAL_SYNTHESIZER_PROMPT + "\n추가 규칙: 최종 결과는 AI agent backend가 구현해야 할 action loop, state/memory requirement, observable trace, forum artifact 생성을 우선 정리하되, 먼저 Program Goal과 Epic Landscape를 제시하고 capability-level epic 후보를 최소 3개 이상 나열하라.\n",
        "round_focus_guide": {
            1: "agent가 forum 안에서 어떤 action loop를 가져야 하는지뿐 아니라 상위 목표와 epic map, state model, content consumption loop, required backend artifacts까지 함께 정의하고 capability 후보를 먼저 나열하라.",
            2: "상태, 기억, 관계, 노출, 행동 스키마와 action 선택 규칙을 구체화하되, 이 slice가 전체 epic 지도에서 어디에 속하는지와 어떤 capability 후보를 대체하는지 함께 적어라.",
            3: "agent의 경향성 형성, belief 변화, 실패 시나리오, 하드코딩처럼 보이는 위험, 인접 epic 의존성을 점검하되, 구현보다 capability 경계를 점검하라.",
            4: "반복 실험에 필요한 trace, snapshot, metric, artifact와 함께 남은 epic gap을 압축하되, capability 후보 간 우선순위를 먼저 설명하라.",
            5: "이슈로 옮길 수 있게 AI agent backend 요구사항과 epic 경계를 정리하되, 하위 task는 This Epic이 확정된 뒤에만 내려라.",
        },
        "synthesis_prompt": """위 결과를 바탕으로:
1. forum 안에서 AI agent가 먼저 지원해야 할 action loop를 추출하고
2. capability-level epic 후보를 최소 3개 이상 정리하고
3. 이 backend contract가 속한 상위 목표와 epic landscape를 먼저 정리한 뒤 This Epic을 선택하고
4. 필요한 상태, 기억, 관계, 행동 규칙과 경향성 형성 메커니즘을 정리하고
5. 어떤 trace, snapshot, forum artifact가 있어야 설명 가능하고 디버깅 가능한지 우선순위를 정한 뒤
6. 바로 GitHub Issue로 등록 가능한 실행 초안을 작성하라.
...
""",
        "coordinator_prompt": COMMON_COORDINATOR_PROMPT + "\n추가 규칙: 이번 토론은 development 팀이 현재 mock을 실제 서비스로 전환하는 계획에 집중한다. 추상적인 설계보다 실제 구현 가능한 첫 번째 동작 루프와 전환 단계를 우선한다. 전체 로드맵에서 이 epic이 어디에 속하는지 보여줘야 한다.\n",
        "final_prompt": COMMON_FINAL_SYNTHESIZER_PROMPT + "\n추가 규칙: 최종 결과는 mock-to-service 전환 목표, 첫 번째 구현 범위, 코드베이스 전환 계획을 중심으로 development 계획을 정리하되, 먼저 Program Goal과 Epic Landscape를 제시하라.\n",
        "round_focus_guide": {
            1: "현재 mock이 왜 아직 서비스가 아닌지와 development 팀의 정확한 범위를 정의하고, 적어도 3~5개의 epic 후보를 먼저 제시하라.",
            2: "후보 epic들을 capability 수준으로 비교하여 이번에 먼저 선택할 This Epic을 고르고, 그 이외의 epics는 Adjacent Epics / Dependencies로 남겨라.",
            3: "mock-to-service 전환 과정의 리스크, 기술 부채, 실패 시나리오, 인접 epic 의존성을 점검하되, 구현 상세보다 capability 경계에 집중하라.",
            4: "개발 우선순위, 마이그레이션 순서, 스프린트 계획, 남은 epic gap을 압축하되, 주요 epic 간 관계를 먼저 설명하라.",
            5: "이슈로 옮길 수 있게 mock-to-service 실행 계획과 epic 경계를 정리하되, 하위 task는 This Epic이 확정된 뒤에만 내려라.",
        },
        "synthesis_prompt": """위 결과를 바탕으로:
1. 현재 mock을 실제 서비스로 전환하기 위한 핵심 목표와 성공 기준을 추출하고
2. 최소 3개 이상의 후보 epic과 전체 서비스화 epic landscape를 먼저 정리하고
3. 이 중 하나의 This Epic을 선택하되, 선택 이유와 제외한 epic을 함께 적고
4. 기술 스택 선택과 첫 번째 구현 범위를 정리하고
5. mock-to-service 마이그레이션 순서와 개발 우선순위를 제시하고
6. 바로 GitHub Issue로 등록 가능한 실행 초안을 작성하라.
...
""",
        "coordinator_prompt": COMMON_COORDINATOR_PROMPT + "\n추가 규칙: 이번 토론은 운영자 조직 설계에 집중한다. 포럼 구축보다 컨텐츠 자정, 모니터링, 개선 포인트 도출을 위한 관찰 프레임과 개입 레버를 우선한다. 전체 운영 epic map도 함께 보여줘야 한다. 최소 3개의 capability epic 후보를 먼저 제시하라.\n",
        "final_prompt": COMMON_FINAL_SYNTHESIZER_PROMPT + "\n추가 규칙: 최종 결과는 운영자 조직의 관찰 체계, 자정 정책, 개선 레버, 운영 아티팩트를 우선 정리하되, 먼저 Program Goal과 Epic Landscape를 제시하고 capability-level epic 후보를 최소 3개 이상 나열하라.\n",
        "round_focus_guide": {
            1: "운영자 조직이 먼저 답해야 할 운영 질문과 개입 목표를 정의하고, 전체 운영 epic map과 capability 후보를 먼저 그려라.",
            2: "메트릭, 대시보드, 인터뷰, 트레이스, 정책 레버를 구체화하되, 이 slice가 전체 운영 구조에서 어디에 속하는지와 어떤 capability 후보를 대체하는지 적어라.",
            3: "측정 불가능한 개입, 운영 리스크, 정책 충돌, 잘못된 개선 방향, 인접 epic 의존성을 점검하되, 구현보다 capability 경계를 점검하라.",
            4: "반복 운영 루프와 기능 개선 우선순위, 남은 epic gap을 압축하되, capability 후보 간 우선순위를 먼저 설명하라.",
            5: "이슈로 옮길 수 있게 운영자 조직 설계안과 개선 계획, epic 경계를 정리하되, 하위 task는 This Epic이 확정된 뒤에만 내려라.",
        },
        "synthesis_prompt": """위 결과를 바탕으로:
1. 운영자 조직이 먼저 고정해야 할 관찰 프레임과 개입 목표를 추출하고
2. capability-level 운영 epic 후보를 최소 3개 이상 정리하고
3. 전체 운영 epic landscape를 먼저 정리한 뒤 This Epic을 선택하고
4. 어떤 메트릭, 인터뷰, 트레이스, 자정 정책, 개선 레버가 필요한지 정리하고
5. 어떤 운영 아티팩트와 반복 루프가 있어야 다음 런과 다음 기능 개선을 설계할 수 있는지 우선순위를 정한 뒤
6. 바로 GitHub Issue로 등록 가능한 실행 초안을 작성하라.
