...
- UI/UX workforce는 thread continuity, reply context, compact compose, tag navigation, empty state, navigation coherence, copy tone, visual hierarchy 같은 surface slice를 우선하고 backend simulation contract로 새지 않는다.
"""
...
- "추후 결정" 같은 표현을 쓰지 않는다. 지금 결정 가능한 것은 결정해서 적는다.
      ...
      - society를 선택했다면 topic은 agent가 본 글, 선택한 글, 좋아요/싫어요, 댓글 반응, 외부 콘텐츠 소비가 characteristic, belief, memory를 어떻게 바꾸는지까지 드러내야 한다.
      - service UX가 아직 덜 닫혔다면 ux workforce를 우선 고려하고, thread continuity / reply context / compact compose / tag navigation / empty state / navigation coherence / copy tone / visual hierarchy 같은 surface slice를 먼저 고른다.
...
최종 출력 형식:
## Selected Workforce
(society / operator / ux / core / default 중 하나)
...
- 여러 라운드에서 충돌한 의견이 있으면 Commitment Critic의 검증 결과를 우선한다.
      ...
      - ux를 선택했다면 Topic에는 thread continuity, reply context, compact compose entrypoint, tag navigation, feed clarity, empty state, navigation coherence, copy tone, visual hierarchy 관점이 직접 드러나야 한다.
...
def choose_catalog_topic(topic: str, workforce: str, context_pack_text: str) -> str:
    ...
    society_identity_guidance = any(
        phrase in guidance_text
        for phrase in [
            "shared content consumption",
            "content consumption",
            "consume and react",
            "react to content",
            "like/dislike",
    ...
    if workforce.lower() == "society" and society_identity_guidance:
        society_identity_priority = [
            "Consumption-driven identity progression minimum",
            "Shared content consumption and identity feedback loop minimum",
            "Reply-driven agent comment generation minimum",
            "Agent action execution contract",
            "State snapshot and memory writeback minimum",
            "Internal/external ingestion envelope",
        ]
        for preferred_topic in society_identity_priority:
            for item in items:
                issue_topic = item.get("issue_topic", "").strip()
                if normalize_topic_text(issue_topic) == normalize_topic_text(preferred_topic):
                    return issue_topic

    surface_ux_guidance = any(
        phrase in guidance_text
        for phrase in [
            "overall ui/ux layout",
            "overall layout",
    ...
    if workforce.lower() == "ux" and surface_ux_guidance:
        ux_priority = [
            "Overall UI/UX layout and information hierarchy minimum",
            "Shared content consumption and identity feedback loop minimum",
            "Threaded reply and comment context minimum",
            "Compact compose entrypoint minimum",
            "Tag navigation and hashtag search minimum",
            "Feed clarity and empty state coherence minimum",
        ...
        for preferred_topic in ux_priority:
            for item in items:
                issue_topic = item.get("issue_topic", "").strip()
                if normalize_topic_text(issue_topic) == normalize_topic_text(preferred_topic):
                    return issue_topic
...
- ux: forum / admin surface UI-UX 완성도 설계 (thread continuity, reply context, compact compose, tag navigation, empty state, navigation coherence, copy tone, visual hierarchy)
...
## 기대 산출물
- Selected Workforce (society / operator / ux / core / default)
...
""",
    ...
    "ux": {
        "label": "UI/UX 완성도 개선",
        "roles_file": SCRIPT_DIR / "ux_roles.yaml",
        "default_topic": "포럼 서비스의 전체 UI/UX 레이아웃과 정보 구조를 먼저 닫기 위해 지금 가장 먼저 정해야 할 최소 slice는 무엇이며, 그다음 shared content consumption / 좋아요/싫어요 / 댓글 반응 / thread continuity / reply context / compact compose entrypoint / tag navigation / feed clarity / empty state / navigation coherence / copy tone / visual hierarchy 중 무엇을 우선해야 하는가?",
        "workforce_description": "AI-Fashion-Forum UI/UX 완성도 개선 토론 팀: UX Product Lead, Interaction Designer, UX Writer, UX Critic이 협력하여 포럼과 admin surface의 사용성 마찰을 먼저 닫습니다.",
        "participants": "UX Product Lead, Interaction Designer, UX Writer, UX Critic",
        "title": "🎨 AI-Fashion-Forum — UI/UX 완성도 토론",
        "arg_description": "CAMEL Workforce 기반 UI/UX 완성도 개선 토론 엔진",
        "build_task_prompt": lambda topic: f"""AI-Fashion-Forum 프로젝트의 forum/admin surface UI/UX 완성도를 토론하세요.

## 토론 주제
{topic}

## 프로젝트 맥락
- 사용자 화면이 반쯤 만들어진 것처럼 보이면 backend simulation보다 먼저 surface UX를 닫아야 한다
- forum 서비스의 첫인상은 먼저 전체 레이아웃과 정보 구조에서 결정되고, 그다음 shared content consumption, 좋아요/싫어요, 댓글 반응, thread continuity, reply context, compact compose entrypoint, tag navigation, feed clarity, empty state, navigation coherence, copy tone, visual hierarchy에서 완성된다
- admin/operator surface도 사용자가 보는 제품의 일부이므로, replay viewer, sprint summary, landing navigation, 카드 배치, 빈 상태가 덜 닫혀 있으면 먼저 고쳐야 한다
- 목표는 "예쁜 화면"이 아니라 "사용자가 실제로 쓰고 싶어지는 포럼 서비스"다
- 이번 토론은 기능 목록이 아니라 capability 수준의 UI/UX epic landscape를 먼저 펼치고, 그중 지금 바로 issue-ready한 가장 작은 surface slice 하나를 남겨야 한다

## 토론 규칙
1. UX Product Lead: 사용자가 실제로 쓰고 싶어지는지 막는 가장 큰 surface gap을 정의하라
2. Interaction Designer: 화면 흐름, 탐색, 상태, 복귀 지점을 설계하라
3. UX Writer: 버튼, 빈 상태, 라벨, 안내 문구의 톤과 명확성을 정리하라
4. UX Critic: backend 문제로 새거나 범위가 커지는 설계를 지적하라
...
## 출력 제약
- 사용자가 실제로 쓰고 싶어지는 포럼 서비스 경험을 우선하라
- shared content consumption, 좋아요/싫어요, 댓글 반응, thread continuity, reply context, compact compose entrypoint, tag navigation, feed clarity, empty state, navigation coherence, copy tone, visual hierarchy를 다루되, 먼저 전체 UI/UX 레이아웃과 정보 구조를 고정하라
- replay viewer, sprint summary, operator/admin surface도 사용자 경험으로서 함께 고려하라
- backend implementation contract로 새지 말고, 화면에서 체감되는 완성도를 먼저 닫아라
- capability-level epic landscape를 먼저 쓰고, 그 다음 가장 작은 UX slice를 issue-ready로 압축하라
""",
        "coordinator_prompt": COMMON_COORDINATOR_PROMPT + "\n추가 규칙: 이번 토론은 UI/UX 완성도에 집중한다. backend contract가 아니라 사용자가 실제로 쓰고 싶어지는 surface slice를 우선하되, 결과는 반드시 하나의 bounded UX slice로 압축하라. thread continuity, reply context, compact compose entrypoint, tag navigation, feed clarity, empty state, navigation coherence, copy tone, visual hierarchy를 놓치지 말고, operator/admin surface도 같은 톤으로 정리하라.\n",
        "final_prompt": COMMON_FINAL_SYNTHESIZER_PROMPT + "\n추가 규칙: 최종 결과는 UI/UX 완성도 epic map과 실제 사용성 마찰을 우선 정리하되, 먼저 Program Goal과 Epic Landscape를 제시하고 capability-level UX epic 후보는 3개 이내로 제한하라. 최종 issue는 지금 바로 발급 가능한 가장 작은 surface slice여야 한다.\n",
        "round_focus_guide": {
            1: "사용자가 실제로 쓰고 싶어지게 만드는 가장 큰 surface gap을 찾되, 먼저 전체 UI/UX 레이아웃과 정보 구조를 고정하고 shared content consumption/반응 루프까지 포함한 이번 run에서 issue-ready로 압축할 수 있는 최소 UX slice를 고르라.",
            2: "화면 흐름, 빈 상태, 복귀 지점, 복사문구, 탐색성을 구체화하되, 나머지는 adjacent epic으로 남겨라.",
            3: "backend로 새는 답변을 비판하고, thread/reply/compose/tag/navigation 중 실제로 가장 먼저 닫아야 할 slice를 확정하라.",
            4: "operator/admin surface를 포함한 시각적 완성도를 다듬되, 실제 issue로 발급할 최소 단위 하나만 남겨라.",
            5: "이슈로 옮길 수 있게 UX epic landscape와 bounded slice를 정리하되, 결과는 단일 issue-ready UX slice여야 한다.",
        },
        "synthesis_prompt": """위 결과를 바탕으로:
1. 사용자가 실제로 쓰고 싶어지는 포럼/관리 화면이 되기 위해 먼저 전체 UI/UX 레이아웃과 정보 구조를 고정한 뒤, shared content consumption/반응 루프를 포함해 닫아야 할 UX gap을 추출하고
2. capability-level UX epic 후보를 3개 이내로 정리하고
3. 전체 UI/UX epic landscape를 간단히 정리한 뒤 This Epic을 선택하되
4. thread continuity, reply context, compact compose entrypoint, tag navigation, feed clarity, empty state, navigation coherence, copy tone, visual hierarchy 중 무엇을 바꿔야 하는지 정리하고
5. replay viewer, sprint summary, operator/admin surface도 사용자 경험으로서 어떻게 정리할지 함께 적되
6. 바로 GitHub Issue로 등록 가능한 가장 작은 surface slice로 압축하라.

Acceptance Criteria는 실제 구현/검증 가능한 체크리스트로 작성하라.
사용자 경험의 첫인상과 사용성 마찰을 우선하라.
""",
    },
}
