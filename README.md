...
- `ux`: 포럼과 admin surface의 UI/UX 완성도를 닫아 사용자가 실제로 쓰고 싶어지는 surface slice를 설계합니다.
...
이 저장소는 의사결정을 세 단계로 분리해서 운영합니다.

1. `commitment`
   - 다음 workforce와 topic만 정합니다.
   - issue 크기나 완료 기준을 최종 확정하지 않습니다.
2. `issue-ready gate`
   - `Issue Title`, `Summary`, `Acceptance Criteria`, `Next Actions`가 충분한지 별도로 판정합니다.
3. `issue issuance`
...
python3 scripts/pipeline/run_studio.py \
  ...
  --sim-results-dir /Users/jongtaelee/Documents/AI-Fashion-Forum/path/to/sim-results \
   --soft-guidance "threads와 Twitter 같은 좋은 포럼 UX를 참고해, ux workforce를 먼저 고려하고 thread continuity, reply context, compact compose entrypoint, tag navigation, feed clarity, empty state, navigation coherence, copy tone, visual hierarchy처럼 사용자가 실제로 쓰고 싶어지는 forum service slice를 우선한다." \
  --rounds 1
```
...
- `build_context.py`는 기본적으로 대상 repo의 issue만 읽습니다. `camel-workforce-studio` 자체의 open issues를 context에 포함하려면 `--include-workspace-issues`를 사용합니다.
...
- issue 발급은 bounded implementation slice에만 적용합니다. 큰 방향은 그대로 issue로 밀지 말고 epic 또는 continuation note로 유지합니다.
